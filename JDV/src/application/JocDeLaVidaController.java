...
//		});
	...
	public void escoltar(Pane p, Cellula c) {
		if(c != null) {
					if(c.isMoribunda()) {
			String fons = context.conversioAHex(Color.LIGHTCYAN);
			p.setStyle("-fx-background-color: " + fons + ";");
		} else if (c.isMorta()) {
			String fons = context.conversioAHex(Color.CYAN);
			p.setStyle("-fx-background-color: " + fons + ";");
		} else if (c.isNaix()) {
		    ...
		    p.setStyle("-fx-background-color: white; -fx-border-color: black;");
